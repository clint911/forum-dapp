...
const Comments = ({ post }: { post: PostDetails }) => {
    const {
        ...
        isLoading,
    }: { data: bigint[] | undefined; isLoading: boolean } = useReadForumGetCommentsFromPost({ address: deployedAddress, args: [post.id], });
    ...
    return (
        <>
            {comments.map((comment) => (
                <ShareableCommentComponent comment={comment} post={post} key={comment.id} />
            ))}
        </>
    )
}
