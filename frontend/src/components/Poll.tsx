...
const Poll = ({ postId }: { postId: bigint }) => {
    ...
    const { data: option1TxHash, writeContractASync: votingOption1 } = useWriteForumUpVotePollOption();
    const { data: option2TxHash, writeContractASync: votingOption2 } = useWriteForumUpVotePollOption();

    const { isLoading: isVotingOption1, isSuccess: hasVotedOption1 } = useWaitForTransactionReceipt({ hash: option1TxHash, })
    const { isLoading: isVotingOption2, isSuccess: hasVotedOption2 } = useWaitForTransactionReceipt({ hash: option2TxHash, })
