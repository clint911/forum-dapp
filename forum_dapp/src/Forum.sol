...
contract Forum {
 //Part A creating a struct for Post, Comment and Poll
   struct Post{
  address owner;
  uint256 id;
  string title;
  ...
  mapping(address => uint256[]) private userPosts;//each ID will be associated to the user associated with post
  mapping(uint256 => Post) public posts;//assign postId to posts
  mapping(uint256 => Poll) public polls;//assign pollId to poll
  mapping(uint256 => Comment) public comments;//assign commentId to comments
  mapping(uint256 => uint256) public postToPoll;//assign postId to pollId
  mapping(uint256 => uint256[]) public postToComments;//postId -> commentId[]
  mapping(address => uint256[]) private userComments;//assign address to a list of comments
  mapping(address => uint256[]) private userPolls;//assign address to list of postIds
...
// Part C - Write Create Comment function
function createPoll(uint256 _postId, string memory _question, string memory _option1, string memory _option2) public returns (uint256) {
...
function createComment(uint256 _postId, string memory _title, string memory _description, bool _spoil) public returns (uint256) {
   require(_postId <= postIdIncrement, "Post does not exist!");
